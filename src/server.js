...
async function sendActionToN8n(payload) {
  ...
  if (n8nActionSecret) {
    headers['x-codigomystery-secret'] = n8nActionSecret;
  }
...
app.post('/webhook/codigomystery-telegram', async (req, res) => {
  ...
  try {
    ...
    console.error('Failed to forward action to n8n', error);
...
app.post('/actions/project', async (req, res) => {
  ...
  try {
    ...
    console.error('Failed to forward project action to n8n', error);
